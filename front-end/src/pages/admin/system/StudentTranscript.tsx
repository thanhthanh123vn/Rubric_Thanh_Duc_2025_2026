...
export default function StudentTranscript() {
    ...
    // ==========================================
    // GIAO DIỆN XEM VÀ IN BẢNG ĐIỂM (HIỆN KHI CLICK NÚT IN)
    // ==========================================
    if (viewingTranscriptStudent) {
        // Dữ liệu giả lập bảng điểm chi tiết của sinh viên được chọn (Có thể thay thế bằng API điểm thật)
        const transcriptDetails = [
            { id: 1, subject: 'Nhập môn lập trình', credits: 3, score: 8.5, letterGrade: 'A' },
            { id: 2, subject: 'Hệ quản trị cơ sở dữ liệu', credits: 3, score: 7.8, letterGrade: 'B+' },
            { id: 3, subject: 'Kỹ thuật phần mềm', credits: 3, score: 8.0, letterGrade: 'B+' },
            { id: 4, subject: 'Nghiên cứu xây dựng hệ thống E-learning', credits: 4, score: 9.2, letterGrade: 'A+' },
        ];
        const totalCredits = transcriptDetails.reduce((sum, item) => sum + item.credits, 0);
        const avgScore = (transcriptDetails.reduce((sum, item) => sum + (item.score * item.credits), 0) / totalCredits).toFixed(2);
        ...
        return (
            <div className="space-y-6 animate-in fade-in duration-300">
                ...
                <div className="flex justify-between items-center print:hidden bg-white p-4 rounded-xl border border-slate-200">
                    ...
                    <Button onClick={handlePrintTrigger} className="bg-emerald-600 hover:bg-emerald-700 text-white">
                        <Printer className="w-4 h-4 mr-2" /> In ngay
                    </Button>
                </div>

                {/* Khung bản in chuẩn A4 */}
                <div className="bg-white p-8 md:p-12 rounded-xl shadow-sm border border-slate-200 print:shadow-none print:border-none print:p-0 max-w-4xl mx-auto">
                    ...
                    <table className="w-full text-sm border-collapse mb-8 border border-slate-800">
                        ...
                        <tr className="bg-slate-100 print:bg-slate-100">
                            ...
                            <th className="border border-slate-800 py-2 px-3 text-center w-24">Điểm hệ 10</th>
                            <th className="border border-slate-800 py-2 px-3 text-center w-24">Điểm chữ</th>
                        </tr>
                        </thead>
                        <tbody>
                        {transcriptDetails.map((item, index) => (
                            <tr key={item.id}>
                                <td className="border border-slate-800 py-2 px-3 text-center">{index + 1}</td>
                                <td className="border border-slate-800 py-2 px-3 font-medium">{item.subject}</td>
                                <td className="border border-slate-800 py-2 px-3 text-center">{item.credits}</td>
                                <td className="border border-slate-800 py-2 px-3 text-center font-semibold">{item.score.toFixed(1)}</td>
                        ...
                        <tr className="font-bold bg-slate-50 print:bg-transparent">
                            ...
                            <td className="border border-slate-800 py-3 px-3 text-center text-blue-700 print:text-black">{avgScore}</td>
