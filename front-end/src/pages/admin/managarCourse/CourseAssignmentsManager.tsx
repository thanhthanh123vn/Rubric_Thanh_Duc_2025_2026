import {
    ...
    FileText, Loader2, ChevronDown, Clock, Paperclip, CheckCircle, Pencil, Trash2, ArrowLeft
} from 'lucide-react';
...
export default function CourseAssignmentsManager() {
        ...
        const fetchCourseName = async () => {
            try {
                if (offeringId) {
                    ...
                    const currentCourse = data.find((c: any) => c.offeringId === offeringId);
                    if (currentCourse) {
                        setCourseName(currentCourse.course.courseName);
                    } else {
                        setCourseName("Không tìm thấy học phần");
                    }
                }
            } catch (error) {
                setCourseName("Lỗi tải tên học phần");
            }
        };
        fetchCourseName();
    }, [offeringId]);
    ...
    return (
        <div className="rounded-[1.5rem] sm:rounded-[2rem] border border-slate-200 bg-white p-4 sm:p-6 shadow-sm">
            {/* Header */}
            <div
                className="flex items-center justify-between mb-8 bg-white p-4 rounded-xl shadow-sm border border-slate-200">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p className="text-sm font-medium text-blue-600 bg-blue-50 px-2 py-0.5 rounded mt-1 inline-block">
                            {courseName}
                        </p>
                    </div>
                </div>
            ...
            <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                <div>
                    <div className="flex items-center gap-2">
                        ...
                        <p className="text-xs sm:text-sm font-semibold uppercase tracking-[0.2em] text-emerald-600">Bài
                            tập</p>
                    </div>
            ...
            {isUploading && (
                <div
                    className="mt-6 rounded-2xl border border-dashed border-emerald-300 bg-emerald-50/50 p-4 sm:p-6 animate-in fade-in slide-in-from-top-2">
                    <div className="flex items-center justify-between mb-4">
                        <h5 className="font-semibold text-emerald-800">
                            {editingId ? "Cập nhật bài tập" : "Thêm bài tập mới"}
                        </h5>
                        <button onClick={handleCloseForm}
                                className="rounded-full p-1 text-slate-400 hover:bg-emerald-100 hover:text-slate-600 transition-colors">
                            <X className="h-5 w-5"/>
                        </button>
                    </div>
                    ...
                    <form className="space-y-4" onSubmit={handleSubmit}>
                        <div>
                            <label className="mb-1.5 block text-sm font-medium text-slate-700">Tên bài tập <span
                                className="text-red-500">*</span></label>
                            <input type="text" name="assessmentName" value={formData.assessmentName}
                        ...
                        <div>
                            <label className="mb-1.5 block text-sm font-medium text-slate-700">Mô Tả <span
                                className="text-red-500">*</span></label>
                            <textarea name="description" value={formData.description} onChange={handleInputChange}
                                      required rows={4}
                                      className="w-full rounded-xl border border-slate-300 px-4 py-2.5 text-sm outline-none focus:border-emerald-500 focus:ring-1 resize-none"/>
                        </div>
                        ...
                        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                            ...
                            <div>
                                <label className="mb-1.5 block text-sm font-medium text-slate-700">Loại hình
                                    thức</label>
                                <select name="assessmentType" value={formData.assessmentType}
                                        onChange={handleInputChange}
                                        className="w-full rounded-xl border border-slate-300 px-4 py-2.5 text-sm outline-none focus:border-emerald-500 focus:ring-1 bg-white">
                        ...
                        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                            ...
                            <div>
                                <label className="mb-1.5 block text-sm font-medium text-slate-700">Chuẩn đầu ra môn học
                                    (CLO)</label>
                                {rubricClos.length > 0 ? (
                                    <div className="flex flex-wrap gap-2 mt-1">
                                        {rubricClos.map(clo => (
                                            <label key={clo.id}
                                                   className="flex items-center gap-2 bg-white border border-emerald-200 px-3 py-1.5 rounded-lg cursor-pointer hover:bg-emerald-50 transition-colors">
                                                <input type="checkbox" value={clo.id}
                                                       checked={selectedClos.includes(clo.id)} onChange={(e) => {
                                    ...
                                    <p className="text-xs text-slate-400 italic mt-2">Vui lòng lựa chọn Rubric chấm điểm
                                        để đồng bộ chuẩn CLO tự động.</p>
                                )}
                            </div>
                            <div>
                                <label className="mb-1.5 block text-sm font-medium text-slate-700">Hạn nộp <span
                                    className="text-red-500">*</span></label>
                        ...
                        {/* File Upload */}
                        <div
                            className="mt-2 flex justify-center rounded-xl border border-dashed border-slate-300 bg-white px-6 py-8">
                            <div className="text-center">
                                <UploadCloud className="mx-auto h-8 w-8 text-slate-400"/>
                                <div className="mt-2 flex text-sm text-slate-600 justify-center">
                                    <label
                                        className="cursor-pointer rounded-md font-semibold text-emerald-600 hover:text-emerald-500">
                                ...
                                <p className="text-xs text-slate-500 mt-1">
                                    {file ? <span
                                        className="font-semibold text-emerald-600">{file.name}</span> : "Tài liệu đính kèm (PDF, DOCX, ZIP tối đa 10MB)"}
                                </p>
                            </div>
                        </div>
                        ...
                        <div className="flex justify-end gap-3 pt-2">
                            ...
                            <button type="submit" disabled={isLoading}
                                    className="flex items-center gap-2 rounded-xl bg-emerald-600 px-4 py-2.5 text-sm font-semibold text-white shadow-sm hover:bg-emerald-700 disabled:opacity-70">
            ...
            <div className="mt-6 space-y-4">
                ...
                {assignments.map((item, idx) => {
                    ...
                    return (
                        <div key={idx}
                             className={`group flex flex-col rounded-2xl border bg-white transition-all duration-200 overflow-hidden ${isExpanded ? 'border-emerald-300 shadow-md ring-2 ring-emerald-50' : 'border-slate-200 hover:border-emerald-200 shadow-sm'}`}>
                            <div onClick={() => toggleExpand(idx)}
                                 className="flex flex-col sm:flex-row sm:items-center justify-between p-4 cursor-pointer hover:bg-slate-50/50 gap-4 sm:gap-0">
                                {/* Trái */}
                                <div className="flex items-start gap-3.5 w-full sm:w-auto">
                                    <div
                                        className={`flex h-12 w-12 shrink-0 items-center justify-center rounded-xl border ${isExpanded ? 'bg-emerald-100 border-emerald-200 text-emerald-700' : 'bg-slate-50 border-slate-100 text-slate-500'}`}>
                                        {typeInfo.icon}
                                    </div>
                                    <div className="flex-1">
                                        ...
                                        <div
                                            className="mt-1.5 flex flex-wrap items-center gap-2 text-[13px] text-slate-500">
                                            <span
                                                className="flex items-center gap-1.5 font-medium bg-slate-100 px-2 py-0.5 rounded-md">
                                                <Clock className="w-3.5 h-3.5"/>
                                                {endDate.toLocaleString('vi-VN')}
                                            </span>
                                            <span
                                                className="font-semibold text-emerald-600 bg-emerald-50 px-2 py-0.5 rounded-md border border-emerald-100">
                                                {item.weight}% trọng số
                                            </span>
                                            {item.rubricId && (
                                                <span
                                                    className="font-semibold text-purple-600 bg-purple-50 px-2 py-0.5 rounded-md border border-purple-100 hidden sm:inline-flex">
                                ...
                                {/* Phải */}
                                <div
                                    className="flex items-center justify-between sm:justify-end gap-3 border-t border-slate-100 sm:border-none pt-3 sm:pt-0 w-full sm:w-auto">
                                    <span
                                        className={`rounded-full px-3 py-1 text-[11px] sm:text-xs font-semibold flex items-center gap-1 ${!isExpired ? 'bg-emerald-50 text-emerald-700' : 'bg-rose-50 text-rose-700'}`}>
                                        {!isExpired ? <CheckCircle className="w-3.5 h-3.5"/> :
                                            <X className="w-3.5 h-3.5"/>}
                                        {!isExpired ? "Đang mở" : "Đã kết thúc"}
                                    </span>

                                    <div className="flex items-center gap-1">
                                        <button onClick={(e) => handleEdit(item, e)} title="Sửa bài tập"
                                                className="rounded-lg p-2 text-slate-400 hover:bg-emerald-100 hover:text-emerald-700 transition-colors">
                                            <Pencil className="h-4 w-4"/>
                                        </button>
                                        <button onClick={(e) => handleDelete(item.assessmentId, e)} title="Xóa bài tập"
                                ...
                                <div className="px-4 pb-5 pt-2 border-t border-slate-100 bg-slate-50/50">
                                    <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm mt-2">
                                        ...
                                        <div className="mt-4 flex flex-wrap gap-2">
                                            <span
                                                className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-blue-50 text-blue-700 text-xs font-semibold border border-blue-100">
                                                {typeInfo.icon}
                                                {typeInfo.label}
                                            </span>
                                            {/* HIỂN THỊ DANH SÁCH CÁC CHUẨN CLO ĐÃ ĐƯỢC LƯU CHO ASSIGNMENT NÀY */}
                                            {item.clos && item.clos.map((clo: any) => (
                                                <span key={clo.id || clo.cloId}
                                                      className="inline-flex items-center px-2.5 py-1 rounded-lg bg-emerald-50 text-emerald-700 text-xs font-bold border border-emerald-100">
                                                    {clo.code || clo.cloId}
                                                </span>
                                            ))}
                                            {item.fileUrl && (
                                                <a href={item.fileUrl} target="_blank" rel="noopener noreferrer"
                                                   ...
                                                   className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-slate-100 text-slate-700 hover:bg-slate-200 text-xs font-semibold border transition-colors">
