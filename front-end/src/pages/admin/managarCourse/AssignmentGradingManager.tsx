...
import { Loader2, BookOpen, Search } from 'lucide-react'; // Đã thêm icon Search
...
export default function AssignmentGradingManager() {
    const navigate = useNavigate();
    const [courses, setCourses] = useState<CourseOfferingResponse[]>([]);
    const [isLoadingCourses, setIsLoadingCourses] = useState(false);

    // 1. State lưu trữ từ khóa tìm kiếm
    ...
    const handleCourseClick = (course: CourseOfferingResponse) => {
        navigate(`/admin/assignments/grading/${course.offeringId}`);
    };

    // 2. Logic lọc khóa học an toàn
        ...
        <div className="p-4 md:p-6 bg-slate-50 min-h-screen">
            <div className="max-w-4xl mx-auto space-y-6">
                ...
                <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
                    <h2 className="text-2xl font-bold text-slate-800 mb-5">Chọn học phần để chấm điểm bài tập</h2>
                        ...
                        {filteredCourses.length > 0 ? (
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                {filteredCourses.map((course) => (
                                    <div
                                        key={course.offeringId || course.course?.courseId}
